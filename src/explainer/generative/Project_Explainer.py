...
class GNN_MOExp(Explainer):
    ...
    def explain(self, instance):
        ...
        y_i = softmax(self.oracle.predict_proba(instance))
        log_y_i = log_softmax(self.oracle.predict_proba(instance))
        ...
        # Prendere il numero di conv layers (L dell'articolo)
        L = self.local_config['oracle'].local_config['parameters']['model']['parameters']['num_conv_layers']
        ...
        # Il primo, cioè il grafo costituito dal solo nodo scelto, è aggiunto separatamente
        ...
        while i<len(G_i_list):
            ...
            log_y_i_prime = log_softmax(self.oracle.predict_proba(subgraph_graph_instance))
